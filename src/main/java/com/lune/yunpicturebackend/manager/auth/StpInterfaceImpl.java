...
public class StpInterfaceImpl implements StpInterface {
    ...
    public SpaceUserAuthContext getAuthContextByRequest() {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest(); // 获取当前请求对象
        String contentType = request.getHeader(Header.CONTENT_TYPE.getValue()); // 获取请求体类型
        ...
        if (ObjUtil.isNotNull(id)) {
            // 获取请求路径业务前缀，如 /api/picture/aaa?id=1
            String requestURI = request.getRequestURI();
            // 先替换上下文，剩下的就是前缀 picture/aaa?id=1
            String partURI = requestURI.replace(contextPath + "/", "");
            // 获取第一个 / 的索引前的字符串 picture
            String modelName = StrUtil.subBefore(partURI, "/", false);
            switch (modelName) {
                case "picture":

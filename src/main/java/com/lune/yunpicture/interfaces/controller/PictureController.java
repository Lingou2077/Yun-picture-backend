package com.lune.yunpicture.interfaces.controller;
...
import com.lune.yunpicture.infrastructure.api.aliyunai.model.GetOutPaintingTaskResponse;
import com.lune.yunpicture.infrastructure.api.imagesearch.ImageSearchApiFacade;
...
import com.lune.yunpicture.interfaces.assemble.PictureAssembler;
...
import com.lune.yunpicturebackend.manager.auth.annotation.SaSpaceCheckPermission;
import com.lune.yunpicturebackend.manager.auth.model.SpaceUserPermissionConstant;
...
import com.lune.yunpicture.interfaces.vo.picture.PictureTagCategory;
...
public class PictureController {
    ...
    /**
     * 构建本地缓存对象 使用Caffeine工具类实现
     */
    private final Cache<String, String> LOCAL_CACHE =
            Caffeine.newBuilder().initialCapacity(1024) // 初始容量
                    .maximumSize(10000L) // 最大缓存数量（10000条）
                    // 缓存 5 分钟移除
     ...
     * 更新图片（仅管理员可用）
     *
     * @param pictureUpdateRequest 修改的图片
     * @param request              请求
     * @return 修改结果
     */
    @PostMapping("/update")
...
//            pictureService.checkPictureAuth(loginUser, picture);
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPage(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        if (spaceId == null) {
            ...
            pictureQueryRequest.setNullSpaceId(true); // 只查询空间 id 为 null
        } else {
            // 私有空间
            boolean hasPermission = StpKit.SPACE.hasPermission(SpaceUserPermissionConstant.PICTURE_VIEW);
            ThrowUtils.throwIf(!hasPermission, ErrorCode.NO_AUTH_ERROR);
...
//            }
    ...
    public BaseResponse<Page<PictureVO>> listPictureVOByPageWithCaChe(@RequestBody PictureQueryRequest pictureQueryRequest,
        ...
        // 缓存逻辑
        // 查询缓存，无数据再查询数据库
        String queryCondition = JSONUtil.toJsonStr(pictureQueryRequest); // 将查询请求体转为 json 字符串 提高可读性
        String hashKey = DigestUtils.md5DigestAsHex(queryCondition.getBytes()); // 使用md5算法加密压缩字符串
        String cacheKey = String.format("yunpicture:listPictureVOByPage:%s", hashKey); // 缓存的 key
        ...
        if (cachedValue != null) {
            // 如果缓存命中，则直接返回缓存数据
            Page<PictureVO> cachedPage = JSONUtil.toBean(cachedValue, Page.class); // 将json字符串转为对象
            return ResultUtils.success(cachedPage);
        }

        // 2. 本地缓存未命中时，操作Redis缓存,从缓存中查询数据
        ValueOperations<String, String> opsForValue = stringRedisTemplate.opsForValue(); // 获取Redis操作对象
        cachedValue = opsForValue.get(cacheKey); // 从缓存中获取数据
        if (cachedValue != null) {
            // 如果缓存命中，更新本地缓存，并返回返回缓存数据
        ...
        Page<Picture> picturePage = pictureApplicationService.page(new Page<>(current, size),
                pictureApplicationService.getQueryWrapper(pictureQueryRequest));
        Page<PictureVO> pictureVOPage = pictureApplicationService.getPictureVOPage(picturePage, request); // 获取封装类
        // 4. 更新缓存
        // 更新redis缓存
        String cacheValue = JSONUtil.toJsonStr(pictureVOPage); // 数据值
        // 设置过期时间，5 - 10 分钟，设置区间随机过期时间避免缓存雪崩
        int cacheTime = 300 + RandomUtil.randomInt(0, 300); // 300 - 600 秒间的随机过期时间
        opsForValue.set(cacheKey, cacheValue, cacheTime, TimeUnit.SECONDS);
        // 更新本地缓存，无需指定时间，已在构造缓存时就指定好了
    ...
    public BaseResponse<Boolean> editPictureBatch(@RequestBody PictureEditByBatchRequest pictureEditBatchRequest, HttpServletRequest request) {
        ...
        User loginUser = userApplicationService.getLoginUser(request);
        pictureApplicationService.editPictureByBatch(pictureEditBatchRequest, loginUser);
        return ResultUtils.success(true);
    }

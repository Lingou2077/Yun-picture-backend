...
import com.lune.yunpicture.interfaces.dto.space.analyze.*;
import com.lune.yunpicture.domain.picture.entity.Picture;
import com.lune.yunpicture.interfaces.vo.space.analyze.*;
...
import com.lune.yunpicture.application.service.SpaceAnalyzeApplicationService;
...
public class SpaceAnalyzeApplicationServiceImpl implements SpaceAnalyzeApplicationService {
    ...
    public SpaceUsageAnalyzeResponse getSpaceUsageAnalyze(SpaceUsageAnalyzeRequest spaceUsageAnalyzeRequest, User loginUser) {
        // 校验参数
        // 全空间或公共图库的数据，需要从Picture表中查询 （需要统计并相加）
        if (spaceUsageAnalyzeRequest.isQueryAll() || spaceUsageAnalyzeRequest.isQueryPublic()) {
            ...
            List<Object> pictureObjList = pictureApplicationService.getBaseMapper().selectObjs(queryWrapper); // 实际只返回一个picSize字段，long类型
            long usedSize = pictureObjList.stream().mapToLong(obj -> (Long) obj).sum(); // 统计已占用的大小
            long usedCount = pictureObjList.size(); // 统计已使用的数量
            ...
            // 如果特定空间数据，则从Space表中查询 （已有空间大小和数量数据）
            // 获取空间数据
    ...
    public List<SpaceTagAnalyzeResponse> getSpaceTagAnalyze(SpaceTagAnalyzeRequest spaceTagAnalyzeRequest, User loginUser) {
        ...
        Map<String, Long> tagCountMap = tagJsonList.stream()
                // 将 ["JAVA","PYTHON"],["JAVA","PHP"] => "JAVA","PYTHON", "JAVA","PHP"
                .flatMap(tagsJson -> JSONUtil.toList(tagsJson, String.class).stream())
                .collect(Collectors.groupingBy(tag -> tag, Collectors.counting()));

        // 转换为响应对象，按照使用次数进行降序排序
    ...
    public List<SpaceUserAnalyzeResponse> getSpaceUserAnalyze(SpaceUserAnalyzeRequest spaceUserAnalyzeRequest, User loginUser) {
        ...
        queryWrapper.eq(ObjUtil.isNotNull(userId), "userId", userId); // 不为空才添加
        // 补充分析维度：每日，每周，每月
    ...
    public List<Space> getSpaceRankAnalyze(SpaceRankAnalyzeRequest spaceRankAnalyzeRequest, User loginUser) {
        ...
        queryWrapper.select("id", "spaceName", "userId", "totalSize")
                ...
                .last("LIMIT " + spaceRankAnalyzeRequest.getTopN()); // 取前N个 （升序）
        // 查询并封装返回结果
    ...
    private void checkSpaceAnalyzeAuth(SpaceAnalyzeRequest spaceAnalyzeRequest, User loginUser) {
        ...
        if (queryAll || queryPublic) {
            ...
            Space space = spaceApplicationService.getById(spaceId);
            ThrowUtils.throwIf(space == null, ErrorCode.NOT_FOUND_ERROR, "空间不存在");
            spaceApplicationService.checkSpaceAuth(loginUser, space); // 校验权限
        }
    }
